 ...
 * @param[in] sample The sample index for _N_ dimensional channels
...
static inline int check_header_contains_channel(const struct sensor_data_generic_header *header,
						enum sensor_channel channel, int sample)
{
	int index = 0;
	...
	for (int i = 0; i < header->num_channels; ++i) {
		if (header->channel_info[i].channel == channel) {
			return index;
		}
		index += SENSOR_CHANNEL_3_AXIS(header->channel_info[i].channel) ? 3 : 1;
	}
	return -1;
}
...
static void sensor_submit_fallback(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	for (size_t i = 0; i < num_channels; ++i) {
		...
		if (rc != 0) {
			LOG_INF("Failed to get channel %d, skipping", channels[i]);
			header->channel_info[i].channel = SENSOR_CHAN_MAX;
		...
		LOG_DBG("header->channel_info[%zu](channel=%d, shift=%d, header_scale=%u)", i,
			channels[i], ilog2(header_scale - 1) + 1, header_scale);
		...
		for (int sample = 0; sample < num_samples; ++sample) {
			...
			int prev_computed_value_idx =
				check_header_contains_channel(header, channels[i], sample);

			if (prev_computed_value_idx >= 0 &&
			    prev_computed_value_idx != current_channel_index + sample) {
				LOG_DBG("value[%d] previously computed at q[%d]@%p", sample,
					prev_computed_value_idx,
					(void *)&q[prev_computed_value_idx]);
				q[current_channel_index + sample] = q[prev_computed_value_idx];
				continue;
			}
			...
			LOG_DBG("value[%d]=%s%d.%06d, q[%d]@%p=%d", sample, value_u < 0 ? "-" : "",
				...
				current_channel_index + sample,
				(void *)&q[current_channel_index + sample],
				q[current_channel_index + sample]);
		}
		current_channel_index += num_samples;
	}
		  ...
		  uint8_t max_count)
{
	...
	while (true) {
		if (compute_channel_info_index(header, *cit, &channel_info_index, &offset) != 0) {
			LOG_INF("Could not compute the channel index, channel iterator may be out "
				"of bounds");
			return -EINVAL;
		}

		if (header->channel_info[channel_info_index].channel == SENSOR_CHAN_MAX) {
			*cit += 1;
		} else {
			break;
		}
