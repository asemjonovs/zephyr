...
static void icm42688_gpio_callback(const struct device *dev, struct gpio_callback *cb,
				   uint32_t pins)
{
	struct icm42688_sensor_data *data = CONTAINER_OF(cb, struct icm42688_sensor_data, gpio_cb);
...
{
	struct icm42688_sensor_data *data = CONTAINER_OF(work, struct icm42688_sensor_data, work);
